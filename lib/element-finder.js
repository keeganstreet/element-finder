module.exports = function(options, progressCallback) {
	...
	begin = function() {
		...
		walk(options.directory, function(err, files) {
			var i;
			if (err) {
				throw err;
			}
			...
			progressCallback({
				...
				'extension' : options.extension.join(', '),
				'ignore' : options.ignore.join(', '),
				'numberOfFiles' : numberOfFiles,
				'message' : 'Searching for "' + options.selector + '" in ' + pluralise(numberOfFiles, 'file', 'files') + ' in "' + options.directory + '".'
			});
